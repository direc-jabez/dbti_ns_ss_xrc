 ...
 * 
 */
define(['N/currentRecord', '/SuiteScripts/utils/NSI_CM_InputDialog', 'N/runtime'],

    function (m_currentRecord, dialog, runtime) {

        // Gloval variables
        var g_currentRecord = null;
        var g_isSubmitBtnClick = false;
        var g_isSignedBtnClick = false;
        ...
        const BILLING_END_DATE_SUBLIST_FIELD_ID = 'custpage_billing_date';
        ...
        const RENTAL_EXHIBIT_ITEM_CODE = '10344';
        const QUANTITY_SUBLIST_FIELD_ID = 'quantity';
        const RATE_SUBLIST_FIELD_ID = 'rate';
        const SECURITY_DEPOSIT_DESCRIPTION_FIELD_ID = 'custbody_xrc_sec_dep_description';
        ...
        const CONSTRUCTION_BOND_PERIOD_FIELD_ID = 'custbody_xrc_construct_period';
        ...
        const CONSTRUCTION_RENT_FREE_PERIOD_FIELD_ID = 'custbody_xrc_constr_rent_free';
        const ADVANCE_CUSA_CHARGES_FIELD_ID = 'custbody_xrc_advance_cusa';
        ...
        const PROPERTY_CLASS_EXHIBIT_ID = '3';
        const PREPARED_BY_FLD_ID = 'custbody_xrc_prepared_by';
        const LEASING_MANAGER_FLD_ID = 'custbody_xrc_approver1';
        const LEASING_HEAD_FLD_ID = 'custbody_xrc_approver2';
        const DOC_STATUS_FLD_ID = 'custbody_xrc_doc_status';
        const PENDING_APPROVAL_ID = '5';
        ...
        var g_mode;
        var g_has_generated = false;
        var g_is_generating = false;
        var g_cusa;
        var g_rental_escalation;
        var g_cusa_escalation;
        ...
        function pageInit(context) {
            ...
            if (context.mode === 'copy') {
                ...
                g_currentRecord.setValue(PREPARED_BY_FLD_ID, currentUser.id);
                g_currentRecord.setValue(LEASING_MANAGER_FLD_ID, null);
                g_currentRecord.setValue(LEASING_HEAD_FLD_ID, null);
                g_currentRecord.setValue(DOC_STATUS_FLD_ID, PENDING_APPROVAL_ID);
                g_currentRecord.setValue(APPROVAL_1_FLD_ID, false);
                g_currentRecord.setValue(APPROVAL_2_FLD_ID, false);
                g_currentRecord.setValue(FOR_APPROVAL_FLD_ID, false);
                g_currentRecord.setValue(REJECTED_FLD_ID, false);
        ...
        function fieldChanged(context) {
            ...
            if (sublistId === ESCALATION_SCHEDULE_SUBLIST_ID) {
                ...
                try {
                    ...
                    if (!g_is_generating) {
                        ...
                        var historical_value_period = getPeriod(historical_values, line);
                    ...
                    if (fieldId === RENTAL_ESCALATION_SUBLIST_FIELD_ID) {
                        ...
                        historical_value_period[RENTAL_ESCALATION_SUBLIST_FIELD_ID] = rental_escalation_percentage;

                        historical_value_period[ESCALATED_BASIC_RENT_SUBLIST_FIELD_ID] = (((100 + rental_escalation_percentage) / 100) * fixed_basic_rent);
                        ...
                        if (isNewYear(period)) {
                            ...
                            if (rental_escalation_percentage) {
                                ...
                                historical_value_period[FIXED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID] = (fixed_basic_rent * period_for_security_deposit);
                                ...
                                historical_value_period[ADJUSTED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID] = (escalated_basic_rent * period_for_security_deposit);
                                ...
                                historical_value_period[SECURITY_DEPOSIT_ADJUSTMENT_SUBLIST_FIELD_ID] = (adjusted_security_deposit ? (adjusted_security_deposit - fixed_security_deposit) : fixed_security_deposit);
                                ...
                                historical_value_period[FIXED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID] = '';
                                ...
                                historical_value_period[ADJUSTED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID] = '';
                                ...
                                historical_value_period[SECURITY_DEPOSIT_ADJUSTMENT_SUBLIST_FIELD_ID] = '';
                        ...
                        historical_value_period[CUSA_ESCALATION_PERCENTAGE_SUBLIST_FIELD_ID] = cusa_escalation_percentage;

                        historical_value_period[ESCALATED_CUSA_SUBLIST_FIELD_ID] = (((100 + cusa_escalation_percentage) / 100) * fixed_cusa);
            ...
            if (fieldId === APPROX_FLR_AREA_SQM_FIELD_ID || fieldId === ACTUAL_RATE_PER_SQM_FIELD_ID) {
                ...
                var periods_for_security_deposit = currentRecord.getValue(PERIODS_FOR_SECURITY_DEPOSIT_FIELD_ID);

                var basic_rent = approx_flr_area_sqm * actual_rate_per_sqm;

                currentRecord.setValue(BASIC_RENT_FIELD_ID, basic_rent);

                currentRecord.setValue(SECURITY_DEPOSIT_FIELD_ID, (basic_rent * periods_for_security_deposit));

            } else if (fieldId === BASIC_RENT_FIELD_ID) {
            ...
            } else if (fieldId === PROPERTY_CLASS_FIELD_ID || fieldId === LEASE_TYPE_FIELD_ID) {

                putItem(currentRecord);

            } else if (fieldId === PERIODS_FOR_SECURITY_DEPOSIT_FIELD_ID) {

                var periods_for_sec_dep = currentRecord.getValue(PERIODS_FOR_SECURITY_DEPOSIT_FIELD_ID);

                // var sec_dep_description = currentRecord.getValue(SECURITY_DEPOSIT_DESCRIPTION_FIELD_ID);

                // sec_dep_description = replaceMonthNumber(sec_dep_description, periods_for_sec_dep);

                // Multiplying Periods for Security Deposit to Security Deposit per period
                // and setting it to Total Security Deposit
                currentRecord.setValue(TOTAL_SECURITY_DEPOSIT_FIELD_ID, periods_for_sec_dep * currentRecord.getValue(SECURITY_DEPOSIT_PER_PERIOD_FIELD_ID));

                // currentRecord.setValue(SECURITY_DEPOSIT_DESCRIPTION_FIELD_ID, sec_dep_description);
            ...
            } else if (fieldId === CONSTRUCTION_BOND_PERIOD_FIELD_ID || fieldId === CONSTRUCTION_BOND_PER_PERIOD_FIELD_ID) {

                // Multiplying Construction (Rent Free) Period to Construction Bond per period
                // and setting it to Total Construction Bond
                currentRecord.setValue(TOTAL_CONSTRUCTION_BOND_FIELD_ID, currentRecord.getValue(CONSTRUCTION_BOND_PERIOD_FIELD_ID) * currentRecord.getValue(CONSTRUCTION_BOND_PER_PERIOD_FIELD_ID));

            } else if (fieldId === CONSTRUCTION_RENT_FREE_PERIOD_FIELD_ID || fieldId === CUSA_FIELD_ID) {

                // Multiplying Construction (Rent Free) Period to Construction Bond per period
                // and setting it to Total Construction Bond
                currentRecord.setValue(ADVANCE_CUSA_CHARGES_FIELD_ID, currentRecord.getValue(CONSTRUCTION_RENT_FREE_PERIOD_FIELD_ID) * currentRecord.getValue(CUSA_FIELD_ID));
            ...
            } else if (fieldId === LEASE_PERIOD_TYPE_FIELD_ID) {

                var lease_period_type = currentRecord.getValue(LEASE_PERIOD_TYPE_FIELD_ID);

                currentRecord.setValue(LEASE_EXPIRY_FIELD_ID, '');

                currentRecord.setValue(LEASE_PERIOD_FIELD_ID, lease_period_type === ONE_TIME_PAYMENT_TYPE_ID ? 1 : '');

                currentRecord.getField(LEASE_PERIOD_FIELD_ID).isDisabled = lease_period_type === ONE_TIME_PAYMENT_TYPE_ID;

                currentRecord.getField(LEASE_EXPIRY_FIELD_ID).isDisabled = lease_period_type !== ONE_TIME_PAYMENT_TYPE_ID;

                currentRecord.getField(PRO_RATED_LEASE_EXPIRTY_FIELD_ID).isDisabled = lease_period_type === ONE_TIME_PAYMENT_TYPE_ID;

            }

            if (fieldId === LEASE_PERIOD_FIELD_ID || fieldId === RENTAL_ESCALATION_PERCENTAGE_FIELD_ID || fieldId === CUSA_FIELD_ID || fieldId === CUSA_ESCALATION_PERCENTAGE_FIELD_ID) {

                if (g_has_generated) {

                    alert('Changing values for Common Use Service Area (CUSA), Rental Escalation % or Cusa Escalation % will require you to re-generate the escalation schedule.');
        ...
        async function onGenerateBtnClick() {
            ...
            var valueCheck = checkValues(lease_period, lease_commencement, basic_rent);
            ...
            if (lease_period_type === ONE_TIME_PAYMENT_TYPE_ID) {
                ...
                value[LEASE_PERIOD_SUBLIST_FIELD_ID] = 1;
                ...
                value[TRUE_BILLING_DATE_SUBLIST_FIELD_ID] = getFifthOfTheMonth(last_date.toLocaleDateString());
                ...
                value[BILLING_START_DATE_SUBLIST_FIELD_ID] = g_currentRecord.getValue(LEASE_COMMENCEMENT_FIELD_ID).toLocaleDateString();

                g_currentRecord.setCurrentSublistValue({
                    sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                    fieldId: BILLING_END_DATE_SUBLIST_FIELD_ID,
                    value: g_currentRecord.getValue(LEASE_EXPIRY_FIELD_ID).toLocaleDateString(),
                });

                value[BILLING_END_DATE_SUBLIST_FIELD_ID] = g_currentRecord.getValue(LEASE_EXPIRY_FIELD_ID).toLocaleDateString();
                ...
                value[FIXED_BASIC_RENT_SUBLIST_FIELD_ID] = basic_rent;
                ...
                value[ESCALATED_BASIC_RENT_SUBLIST_FIELD_ID] = basic_rent;
                ...
                value[ESCALATED_CUSA_SUBLIST_FIELD_ID] = cusa_rent;

                value[FIXED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID] = '';

                value[ADJUSTED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID] = '';

                value[SECURITY_DEPOSIT_ADJUSTMENT_SUBLIST_FIELD_ID] = '';
                ...
                g_currentRecord.setValue(HISTORICAL_VALUES_FIELD_ID, JSON.stringify(values));
            ...
            for (var i = 1; i <= lease_period; i++) {
                ...
                value[YEAR_SUBLIST_FIELD_ID] = year;
                ...
                value[LEASE_PERIOD_SUBLIST_FIELD_ID] = i;
                ...
                value[BILLING_START_DATE_SUBLIST_FIELD_ID] = (i === 0) ? g_currentRecord.getValue(LEASE_COMMENCEMENT_FIELD_ID).toLocaleDateString() : getFirstDateOfMonth(last_date.getFullYear(), last_date.getMonth()).toLocaleDateString();

                value[BILLING_END_DATE_SUBLIST_FIELD_ID] = (i === lease_period && pro_rated_lease_expiry) ? g_currentRecord.getValue(LEASE_EXPIRY_FIELD_ID).toLocaleDateString() : last_date.toLocaleDateString();
                ...
                if (year > 1) {
                    ...
                    value[RENTAL_ESCALATION_SUBLIST_FIELD_ID] = rental_escalation;
                    ...
                    value[CUSA_ESCALATION_PERCENTAGE_SUBLIST_FIELD_ID] = cusa_escalation;

                } else {

                    value[RENTAL_ESCALATION_SUBLIST_FIELD_ID] = "";
                    value[CUSA_ESCALATION_PERCENTAGE_SUBLIST_FIELD_ID] = "";
                ...
                value[FIXED_BASIC_RENT_SUBLIST_FIELD_ID] = fbr;
                ...
                value[ESCALATED_BASIC_RENT_SUBLIST_FIELD_ID] = ebr;
                ...
                if (i === 0 || i === lease_period) {
                    ...
                    fc = getProRatedRent(cusa_rent, last_date, day);
                ...
                value[FIXED_CUSA_SUBLIST_FIELD_ID] = fc;
                ...
                value[ESCALATED_CUSA_SUBLIST_FIELD_ID] = ec;
                ...
                if (isNewYear(i)) {
                    ...
                    g_currentRecord.setCurrentSublistValue({
                        sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                        fieldId: FIXED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID,
                        value: value[FIXED_BASIC_RENT_SUBLIST_FIELD_ID] * period_for_security_deposit,
                    });

                    value[FIXED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID] = (fixed_basic_rent * period_for_security_deposit);
                    ...
                    value[ADJUSTED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID] = (escalated_basic_rent * period_for_security_deposit);
                    ...
                    value[SECURITY_DEPOSIT_ADJUSTMENT_SUBLIST_FIELD_ID] = sda;
                ...
                if (i === 0) {
                    ...
                    // i !== 0 && ((lease_period === 1 && i === 1) || (i > 1 && i % 12 === 0) || (lease_period < 12 && i === lease_period - 1))
                } else if (i !== 0 && ((lease_period === 1 && i === 1) || (isNewYear(i)))) {
        ...
        function canAddLine(i, lease_period) {

            return i === 0 || (i !== 0 && ((lease_period === 1 && i === 1) || (i > 1 && i % 12 === 0) || (lease_period < 12 && i === lease_period - 1)));

        }

        function onRemoveAllLinesBtnClick() {
        ...
        async function onRecalcBtnClick() {
            ...
            var historical_values = JSON.parse(g_currentRecord.getValue(HISTORICAL_VALUES_FIELD_ID));

            var lease_commencement = g_currentRecord.getValue(LEASE_COMMENCEMENT_FIELD_ID);

            var pro_rated_lease_expiry = g_currentRecord.getValue(PRO_RATED_LEASE_EXPIRTY_FIELD_ID);

            var basic_rent = g_currentRecord.getValue(BASIC_RENT_FIELD_ID) || 0;

            var cusa = g_currentRecord.getValue(CUSA_FIELD_ID) || 0;

            // var current_lease_year = (g_currentRecord.getValue(CURRENT_LEASE_YEAR_FIELD_ID)).toString() || "0";

            var lease_period = g_currentRecord.getValue(LEASE_PERIOD_FIELD_ID);

            var current_lease_period = 0;
            ...
            var escalation_lines = historical_values.length;

            var period_for_security_deposit = g_currentRecord.getValue(PERIODS_FOR_SECURITY_DEPOSIT_FIELD_ID);

            // Separating variables for each column so it won't conflict with each other columns
            var current_year = 1, current_cusa_year = 1, current_rent_escalation = 0, current_cusa_escalation = 0;
            ...
            var last_year = parseInt(historical_values[historical_values.length - 1]['custpage_year']);

            for (let year = 1; year <= last_year; year++) {

                let year_periods = historical_values.filter(historical_value => historical_value.custpage_year === year);

                let startIndex = year_periods.length === 13 ? 1 : 0;

                for (let j = startIndex; j < year_periods.length; j++) {

                    year_periods[j]['custpage_fixed_basic_rent'] = year_periods[startIndex]['custpage_fixed_basic_rent'];
                    year_periods[j]['custpage_rental_escalation'] = year_periods[startIndex]['custpage_rental_escalation'];
                    year_periods[j]['custpage_escalated_basic_rent'] = year_periods[startIndex]['custpage_escalated_basic_rent'];
                    year_periods[j]['custpage_fixed_cusa'] = year_periods[startIndex]['custpage_fixed_cusa'];
                    year_periods[j]['custpage_cusa_escalation'] = year_periods[startIndex]['custpage_cusa_escalation'];
                    year_periods[j]['custpage_escalated_cusa'] = year_periods[startIndex]['custpage_escalated_cusa'];

                }

                // console.log(year_periods);

                try {

                    let next_year_period = historical_values.filter(historical_value => historical_value.custpage_year === year + 1)[0];

                    console.log(next_year_period);

                    // console.log(next_year_period['custpage_fixed_basic_rent']);

                    next_year_period['custpage_fixed_basic_rent'] = year_periods[year_periods.length - 1]['custpage_escalated_basic_rent'];
                    next_year_period['custpage_escalated_basic_rent'] = (((100 + next_year_period['custpage_rental_escalation']) / 100) * next_year_period['custpage_fixed_basic_rent']);
                    next_year_period['custpage_fixed_cusa'] = year_periods[year_periods.length - 1]['custpage_escalated_cusa'];
                    next_year_period['custpage_escalated_cusa'] = (((100 + next_year_period['custpage_cusa_escalation']) / 100) * next_year_period['custpage_fixed_cusa']);
                    next_year_period['custpage_fixed_sec_dep'] = (next_year_period['custpage_fixed_basic_rent'] * period_for_security_deposit); // 4 => Periods for Security Deposit
                    next_year_period['custpage_adj_sec_dep'] = (next_year_period['custpage_escalated_basic_rent'] * period_for_security_deposit); // 4 => Periods for Security Deposit
                    next_year_period['custpage_sec_dep_adj'] = (next_year_period['custpage_adj_sec_dep'] - next_year_period['custpage_fixed_sec_dep']);
            ...
            g_currentRecord.setValue(HISTORICAL_VALUES_FIELD_ID, JSON.stringify(historical_values));

            var escalation_lines = g_currentRecord.getLineCount({ sublistId: ESCALATION_SCHEDULE_SUBLIST_ID });

            for (var line = 0; line < escalation_lines; line++) {
                ...
                if (year > 0) {
                    ...
                    var historical_value_period = historical_values.filter(historical_value => historical_value.custpage_year == year);

                    if (historical_value_period.length === 13) {
                        historical_value_period = historical_value_period[1];
                    } else {
                        historical_value_period = historical_value_period[0];
                    ...
                    g_currentRecord.setCurrentSublistValue({
                        sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                        fieldId: FIXED_BASIC_RENT_SUBLIST_FIELD_ID,
                        value: historical_value_period[FIXED_BASIC_RENT_SUBLIST_FIELD_ID],
                    });

                    g_currentRecord.setCurrentSublistValue({
                        sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                        fieldId: RENTAL_ESCALATION_PERCENTAGE_FIELD_ID,
                        value: historical_value_period[RENTAL_ESCALATION_PERCENTAGE_FIELD_ID] || 0,
                    });

                    g_currentRecord.setCurrentSublistValue({
                        sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                        fieldId: ESCALATED_BASIC_RENT_SUBLIST_FIELD_ID,
                        value: historical_value_period[ESCALATED_BASIC_RENT_SUBLIST_FIELD_ID],
                    });

                    g_currentRecord.setCurrentSublistValue({
                        sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                        fieldId: FIXED_CUSA_SUBLIST_FIELD_ID,
                        value: historical_value_period[FIXED_CUSA_SUBLIST_FIELD_ID],
                    });

                    g_currentRecord.setCurrentSublistValue({
                        sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                        fieldId: CUSA_ESCALATION_PERCENTAGE_FIELD_ID,
                        value: historical_value_period[CUSA_ESCALATION_PERCENTAGE_FIELD_ID] || 0,
                    });

                    g_currentRecord.setCurrentSublistValue({
                        sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                        fieldId: ESCALATED_CUSA_SUBLIST_FIELD_ID,
                        value: historical_value_period[ESCALATED_CUSA_SUBLIST_FIELD_ID],
                    });

                    g_currentRecord.setCurrentSublistValue({
                        sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                        fieldId: FIXED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID,
                        value: historical_value_period[FIXED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID],
                    });

                    g_currentRecord.setCurrentSublistValue({
                        sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                        fieldId: ADJUSTED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID,
                        value: historical_value_period[ADJUSTED_SECURITY_DEPOSIT_SUBLIST_FIELD_ID],
                    });

                    g_currentRecord.setCurrentSublistValue({
                        sublistId: ESCALATION_SCHEDULE_SUBLIST_ID,
                        fieldId: SECURITY_DEPOSIT_ADJUSTMENT_SUBLIST_FIELD_ID,
                        value: historical_value_period[SECURITY_DEPOSIT_ADJUSTMENT_SUBLIST_FIELD_ID],
                    });
        ...
        function putItem(currentRecord) {
            ...
            ['item', 'quantity', 'rate'].forEach((field, index) => {
                g_currentRecord.setCurrentSublistValue({
                    sublistId: ITEMS_SUBLIST_ID,
                    fieldId: field,
                    value: field === 'item' ? currentRecord.getValue(PROPERTY_CLASS_FIELD_ID) === PROPERTY_CLASS_EXHIBIT_ID ? RENTAL_EXHIBIT_ITEM_CODE : (values[index] === LEASE_TYPE_VARIABLE_RENT_ID ? VARIABLE_RENT_ITEM_CODE : FIXED_RENT_ITEM_CODE) : values[index],
        ...
        function updateRentItem(currentRecord) {
            ...
            for (var line = 0; line < items_lines; line++) {
                ...
                if (item === FIXED_RENT_ITEM_CODE || item === VARIABLE_RENT_ITEM_CODE || item === RENTAL_EXHIBIT_ITEM_CODE) {

                    var basic_rent = currentRecord.getValue(BASIC_RENT_FIELD_ID);

                    // Update the value with the current basic rent value
                    currentRecord.setCurrentSublistValue({
                        sublistId: ITEMS_SUBLIST_ID,
        ...
        function onInitialSOBtnClick() {

            var currentRecord = m_currentRecord.get();

            // Redirect to Initial SOA record
            window.location.href = 'https://9794098.app.netsuite.com/app/common/custom/custrecordentry.nl?rectype=1222&origin_id=' + currentRecord.id;

        }

        function onLeaseMemorandumBtnClick() {

            var currentRecord = m_currentRecord.get();

            // Redirect to Initial SOA record
            window.location.href = 'https://9794098.app.netsuite.com/app/accounting/transactions/salesord.nl?memdoc=0&transform=estimate&e=T&id=' + currentRecord.id + '&whence=';
        ...
        function checkValues(lease_period, lease_commencement, basic_rent) {

            // Defining array for paramerters and labels
            var params_labels = ['Lease Period', 'Lease Commencement', 'Basic Rent'];

            var params = [lease_period, lease_commencement, basic_rent];
        ...
        function getPeriod(historical_values, line) {

            var periods = historical_values.filter((historical_value) => historical_value.custpage_year == line);
        ...
        function onSubmitBtnClick() {
            ...
            if (!g_isSubmitBtnClick) {
                ...
                location.href = "https://9794098.app.netsuite.com/app/site/hosting/restlet.nl?script=1525&deploy=1&action=submit&id=" + currentRecord.id;
        ...
        function onSignedByTenantBtnClick() {

            var currentRecord = m_currentRecord.get();

            // Check if the button is already clicked
            // This is to prevent calling the link multiple times
            if (!g_isSignedBtnClick) {

                g_isSignedBtnClick = true;

                var options = {
                    title: 'Attachment',
                    message: 'Please input attachment url below:',
                    buttons: [{
                        label: 'Okay',
                        value: 1
                    }, {
                    ...
                    textarea: {
                        ...
                        isMandatory: true,
                        caption: 'Attachment',
                        fieldId: 'custbody_xrc_lp_attachment',
                        actionButtons: [1]
                    }
                };
                ...
                var success = function (result, value) {
                    ...
                    if (result === 1) {
                        ...
                        // Redirect to the approval link
                        location.href = "https://9794098.app.netsuite.com/app/site/hosting/restlet.nl?script=1525&deploy=1&action=signed&id=" + currentRecord.id + "&url=" + value;

                    } else if (result === 2) {

                        g_isSignedBtnClick = false;
        ...
        function onSubmitForApprovalBtnClick(role_to_email) {
            ...
            if (!g_isSubmitForApprovalBtnClick) {
                ...
                // Redirect to the approval link
                window.location.href = "https://9794098.app.netsuite.com/app/site/hosting/restlet.nl?script=1525&deploy=1&action=submitforapproval&id=" + currentRecord.id + "&role_to_email=" + role_to_email;
        ...
        function onApproveBtnClick(field, role_to_email) {
            ...
            if (!g_isApproveBtnClick) {
                ...
                // Redirect to the approval link
                location.href = "https://9794098.app.netsuite.com/app/site/hosting/restlet.nl?script=1525&deploy=1&action=approve&id=" + currentRecord.id + "&field=" + field + "&role_to_email=" + role_to_email;
        ...
        function onRejectBtnClick() {
            ...
            if (!g_isRejectBtnClick) {
                ...
                // Redirect to the approval link
                location.href = "https://9794098.app.netsuite.com/app/site/hosting/restlet.nl?script=1525&deploy=1&action=reject&id=" + currentRecord.id;
        ...
        function onCancelBtnClick() {
            ...
            if (!g_isCancelBtnClick) {
                ...
                // Redirect to the approval link
                location.href = "https://9794098.app.netsuite.com/app/site/hosting/restlet.nl?script=1525&deploy=1&action=cancel&id=" + currentRecord.id;
        ...
        // To replace numbers/months in description fields
        function replaceMonthNumber(text, newNumber) {
            return text.replace(/\(\d+\)/, `(${newNumber})`);
        }
        ...
        return {
            ...
            onInitialSOBtnClick: onInitialSOBtnClick,
            onLeaseMemorandumBtnClick: onLeaseMemorandumBtnClick,
            onSubmitBtnClick: onSubmitBtnClick,
            onSignedByTenantBtnClick: onSignedByTenantBtnClick,
